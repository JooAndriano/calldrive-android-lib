...
if [ $3 = $stableBranch ]; then
    ...
    curl -u $4:$5 -X PUT https://calldrive.kaminsky.me/remote.php/webdav/$repository-findbugs/$stableBranch.html --upload-file build/reports/spotbugs/spotbugs.html
    ...
    curl -u $4:$5 -X PUT -d "$summary" https://calldrive.kaminsky.me/remote.php/webdav/$repository-findbugs/findbugs-summary-$stableBranch.html

    if [ $lintValue -ne 1 ]; then
        echo "New lint result for $stableBranch at: https://www.kaminsky.me/nc-dev/$repository-lint/$stableBranch.html"
        curl -u $4:$5 -X PUT https://calldrive.kaminsky.me/remote.php/webdav/$repository-lint/$stableBranch.html --upload-file build/reports/lint/lint.html
        exit 0
    fi
else
    if [ -e $6 ]; then
    ...
    curl 2>/dev/null -u $4:$5 -X PUT https://calldrive.kaminsky.me/remote.php/webdav/$repository-lint/$6.html --upload-file build/reports/lint/lint.html

    echo "New SpotBugs results at https://www.kaminsky.me/nc-dev/$repository-findbugs/$6.html"
    curl 2>/dev/null -u $4:$5 -X PUT https://calldrive.kaminsky.me/remote.php/webdav/$repository-findbugs/$6.html --upload-file build/reports/spotbugs/spotbugs.html

    # delete all old comments
    oldComments=$(curl 2>/dev/null -u $1:$2 -X GET https://api.github.com/repos/calldrive/android-library/issues/$7/comments | jq '.[] | (.id |tostring) + "|" + (.user.login | test("calldrive-android-bot") | tostring) ' | grep true | tr -d "\"" | cut -f1 -d"|")

    echo $oldComments | while read comment ; do
        curl 2>/dev/null -u $1:$2 -X DELETE https://api.github.com/repos/calldrive/android-library/issues/comments/$comment
    done
    ...
    lintResultOld=$(curl 2>/dev/null https://raw.githubusercontent.com/calldrive/android-library/$stableBranch/scripts/analysis/lint-results.txt)
    ...
    curl -u $1:$2 -X POST https://api.github.com/repos/calldrive/android-library/issues/$7/comments -d "{ \"body\" : \"$lintResult $findbugsResultNew $findbugsResultOld $lintMessage $findbugsMessage \" }"
